     ...
     with col2:
       ...
       if classify:
         ...
         with st.spinner('Please wait! Making predictions on patches using the SVC model...'):
           ...
           st.write(
                  ':blue-background[The classes (species) assigned to each image patch are listed in the table. Species]', f" :red-background[*{class_predict}*]" , 'was assigned to ', f":blue[*{df['Patches'][0]}*]", ':blue[patches] out of a total of 50, with sum and mean maximum probabilities equal to', f":blue[*{round(df['Sum'][0],2)}*]" ,'and', f":blue[*{x}*],", 'respectively. Therefore, the image is classified as belonging to species', f":blue[*{class_predict}*]", '.'
                  )
